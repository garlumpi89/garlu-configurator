...
let port=null,demoMode=false,currentPage=0,validationIssues=[],suppressTopValidation=false;
...
function syncSegmentedControls(){$("ringBrightness").addEventListener("input",()=>{config.ringBrightness=normalizeRing($("ringBrightness").value);updateRingSlider()});document.querySelectorAll(".segmented").forEach(g=>{const target=$(g.dataset.target);if(!target)return;g.querySelectorAll("button").forEach(b=>b.classList.toggle("active",b.dataset.value===target.value))})}
function updateUiFromConfig(){$("screenLayout").value=["standard","performance"].includes(config.screenLayout)?config.screenLayout:"standard";$("resolutionMode").value=config.highResolution===true?"enhanced":"midi1";$("oledBrightness").value=["low","medium","high"].includes(config.oledBrightness)?config.oledBrightness:"medium";$("ringBrightness").value=["low","medium","high"].includes(config.ringBrightness)?config.ringBrightness:70;for(let i=0;i<4;i++){const input=$(`cc${i}`);input.max=maxAllowedCC();input.min=0;input.value=config.pages[currentPage]?.cc?.[i]??""}updateDeviceLabels();updateValidationHighlights();syncSegmentedControls();syncCustomSelects()}
function updateConfigFromUi(){suppressTopValidation=false;config.screenLayout=$("screenLayout").value;config.highResolution=$("resolutionMode").value==="enhanced";config.oledBrightness=$("oledBrightness").value;config.ringBrightness=normalizeRing($("ringBrightness").value);updateRingSlider();for(let i=0;i<4;i++){const raw=$(`cc${i}`).value;config.pages[currentPage].cc[i]=raw===""?0:Number(raw)}setValidationIssues(validateConfig(config))}
...
function importConfigFromRawText(rawText,label="JSON"){suppressTopValidation=true;try{const p=JSON.parse(rawText);if(!p.pages||p.pages.length!==4)throw new Error("Invalid pages");config.device=p.device||"GARLU_FADER_MINI";config.fw=p.fw||config.fw||"imported";config.screenLayout=p.screenLayout;config.highResolution=p.highResolution;config.oledBrightness=p.oledBrightness;config.ringBrightness=p.ringBrightness;config.pages=JSON.parse(JSON.stringify(p.pages));currentPage=0;document.querySelectorAll(".page").forEach(b=>b.classList.remove("active"));document.querySelector('.page[data-page="0"]').classList.add("active");const issues=validateConfig(config);setValidationIssues(issues);updateUiFromConfig();if(issues.length){setJsonWarnings(issues.map(i=>i.message));setOutputText(rawText,true);toast(`${label} has warnings`);return false}setJsonWarnings([]);setOutputText(JSON.stringify(config,null,2));suppressTopValidation=false;setValidationIssues([]);toast(`${label} applied`);return true}catch{const m="Invalid JSON syntax or GARLU structure. Check commas, quotes and boolean values.";setValidationIssues([{type:"structure",message:m}]);setJsonWarnings([m]);setOutputText(rawText,true);toast("Invalid JSON");return false}}
...
function labelForSelectValue(id, value) {
  const labels = {
    ...
    oledBrightness: { low: "Low", medium: "Medium", high: "High" }
  };
